        ...
        <div class="top">
            ...
            <div class="actions">
                @if ($job->vendor->user_id = Auth::user()->id)
                    ...
                    {{-- <a class="btn btn-primary mr-2" href="{{route('user.dispute.mitigate', $dispute->id)}}">Pay for mitigation</a> --}}
                    {{-- <a class="btn btn-danger" href="{{route('user.dispute.cancel', $dispute->id)}}">Cancel</a> --}}
                @endif

                <a class="btn btn-secondary" style="border: 1px solid white;" href="{{route('user.dispute.back.two', $dispute->id)}}">Return to chat</a>
    ...
    <div class="row">
        @if($mitigation == null)
            <div class="col-md-5 pay-card">
                <p class="text-center">
                    Remaining Milestone Payment: <span>${{$milestone->amt_due}}</span><br>
                    Mitigation Service Charge: <span>${{$dispute->settlement_amount}}</span>

                </p>

                <div class="box">
                    <p class="text-center">
                        Total Amount Under Dispute <br/><br/> <b style="color: #6F3C96; font-size:2rem">${{$dispute->settlement_amount + $milestone->amt_due}}</b>
                    </p>
                </div>

                <div class="btn-flex">

                    <form action="{{route('user.dispute.mitigation.pay')}}" method="post">
                        @csrf
                        <input type="hidden" name="amt" value="{{$dispute->settlement_amount + $milestone->amt_due}}">
                        <input type="hidden" name="dispute_id" value="{{$dispute->id}}">
                        <button class="btn btn-outline-primary mr-2">Pay for mitigation</button>
                    </form>

                    <form action="{{route('user.dispute.drop', $dispute->id)}}" method="get">
                        @csrf
                        <button class="btn btn-secondary">Pay Milestone</button>
                    </form>
