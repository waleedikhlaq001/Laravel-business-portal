...
<style>
    ...
    #uploadProduct::-webkit-scrollbar {
        display: none;
    }
</style>
@endpush
...
<div class="container-fluid">
    <div class="contactFreelancersContainer">
        <h6>Tell us what you need done</h6>
        <p class="mt-4">
            Contact skilled creatives within minutes. View profiles, ratings, portfolios and chat with them.
            Pay the creative only when you are 100% satisfied with their work.
        </p>
    </div>

    <div class="container">
        <div class="jbContainer bg-white p-4 shadow-sm" style="margin-top: -10rem;">
            @if (count($products) > 0)
            <form action="{{route('user.vendor.jobs.create')}}" id="jobForm" name="mf" method="POST" enctype="multipart/form-data">
                @csrf
                <h6>Choose a name for your project <span class="required">*</span></h6>
                <div class="mt-4 mb-4">
                    <div class="form-group">
                        <input type="text" class="form-control cFFormControl title @error('title') is-invalid @enderror" name="title" placeholder="e.g. I need video for my cosmetics" value="{{old('title')}}" maxlength="50">
                        @error('title')
                ...
                <h6 class="mb-0">Give some more details about your Job. <span class="required">*</span></h6>
                <small>Let the Creative know everything you can about your product/service/brand and how you want them to let the world know about it. <br> <b>(*Please enter at least 30 characters*)</b></small>
                <div class="mt-4 mb-5">
                    <div class="form-group">
                        {{-- <input type="text" class="form-control cFFormControl" placeholder="Describe your project here.."> --}}
                        <textarea name="description" id="" cols="30" rows="3" class="form-control desc @error('description') is-invalid @enderror" placeholder="Describe your project here.." maxlength="8000">{{old('description')}}</textarea>
                        @error('description')
                        <div class="invalid-feedback">
                            {{$message}}
                        </div>
                ...
                <h6>I want my content to be ( choose at least 1 option ): <span class="required">*</span></h6>
                <div class="mt-2 mb-4">
                    <div class="form-group">
                ...
                <div class="cFFilesContainer mt-4 mb-2 mx-1 row">
                        ...
                        <div class="d-flex w-100">
                            <div class="btn btn-primary fileBtnUpload d-flex w-100 justify-content-center">
                                <svg width="30" height="40" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M50.9679 31.3258C50.3248 31.3258 49.8102 31.8403 49.8102 32.4835V43.0139C49.8102 45.8781 47.4778 48.202 44.6222 48.202H19.2221C16.358 48.202 14.0341 45.8695 14.0341 43.0139V32.312C14.0341 31.6688 13.5196 31.1543 12.8764 31.1543C12.2333 31.1543 11.7188 31.6688 11.7188 32.312V43.0139C11.7188 47.1558 15.0888 50.5173 19.2221 50.5173H44.6222C48.7641 50.5173 52.1256 47.1472 52.1256 43.0139V32.4835C52.1256 31.8489 51.6111 31.3258 50.9679 31.3258Z" fill="#6F3C96" />
                                    <path d="M25.3764 22.4088L30.7617 17.0235V40.5113C30.7617 41.1544 31.2762 41.6689 31.9194 41.6689C32.5625 41.6689 33.077 41.1544 33.077 40.5113V17.0235L38.4623 22.4088C38.6853 22.6318 38.9854 22.7518 39.277 22.7518C39.5771 22.7518 39.8687 22.6404 40.0916 22.4088C40.5461 21.9543 40.5461 21.2254 40.0916 20.7709L32.734 13.4133C32.5197 13.1989 32.2195 13.0703 31.9194 13.0703C31.6107 13.0703 31.3191 13.1904 31.1047 13.4133L23.7471 20.7709C23.2926 21.2254 23.2926 21.9543 23.7471 22.4088C24.193 22.8547 24.9305 22.8547 25.3764 22.4088Z" fill="#6F3C96" />
                                    <rect x="1" y="1" width="62" height="62" rx="14" stroke="#6F3C96" stroke-width="2" stroke-miterlimit="3.86874" stroke-dasharray="12 12" />
                                </svg>
                                <h6 class="ml-3 mt-1">Upload file</h6>
                                <input type="file" name="documents[]" id="file" accept=".doc, .docx, .pdf, image/jpeg, image/png, image/jpg" multiple>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-12 col-lg-9 mt-2">
                        <div class="flRMVDOC text-danger" style="margin-top: -.8rem">
                            <i class="fa fa-times" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="Remove files"></i>
                        </div>
                ...
                <div class="form-group mt-4">
                    <h6>Select a product <span class="required">*</span></h6>
                    <select name="product_id" class="form-control form-select bg-transparent" id="">
                        <option value="" selected disabled>Select Product</option>
                        @foreach ($products as $product)
                        <option value="{{$product->id}}" @if(old('product_id')==$product->id) {{'selected'}}
                            @endif>{{$product->name}} - {{$product->unique_id}}</option>
                        @endforeach
                    </select>
                    <small>
                        Only approved products can be seleted.
                    </small>
                    <small>
                        Don't have a product to sell? <a href="javascript:void(0);" data-toggle="modal" data-target="#uploadProduct" class="text-snd" target="_blank">Upload
                            a product now</a>
                    </small>
                </div>
                    ...
                    <div class="col-12 col-sm-12 col-md-6 col-xl-6 mt-2 f">
                        <div class="pSSContainer" id="fixed">
                            ...
                            <svg width="68" height="48" viewBox="0 0 68 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M5.6503 18.0881C6.20147 18.0881 6.64838 17.6412 6.64838 17.0901V4.8482C6.64838 3.28632 7.919 2.01557 9.48101 2.01557H58.5223C60.0842 2.01557 61.3549 3.28619 61.3549 4.8482V12.2995C61.3549 12.8507 61.8017 13.2976 62.353 13.2976C62.9043 13.2976 63.3511 12.8507 63.3511 12.2995V4.8482C63.3511 2.18557 61.1849 0.0195312 58.5224 0.0195312H9.48101C6.81838 0.0195312 4.65234 2.1857 4.65234 4.8482V17.0901C4.65234 17.6414 5.09912 18.0881 5.6503 18.0881Z" fill="white" />
                                <path d="M67.0019 40.1593H63.3493V16.2918C63.3493 15.7407 62.9024 15.2938 62.3512 15.2938C61.8 15.2938 61.3531 15.7407 61.3531 16.2918V40.1593H58.8727V33.9677C58.8727 33.4165 58.4258 32.9696 57.8746 32.9696C57.3235 32.9696 56.8766 33.4165 56.8766 33.9677V40.1593H40.1276C39.5764 40.1593 39.1295 40.6062 39.1295 41.1574V41.9252C39.1295 42.1168 38.9736 42.2727 38.7819 42.2727H26.5444C26.3528 42.2727 26.1969 42.1168 26.1969 41.9252V41.1574C26.1969 40.6062 25.75 40.1593 25.1988 40.1593H11.1232V6.49227H56.8767V29.9755C56.8767 30.5267 57.3236 30.9736 57.8748 30.9736C58.4259 30.9736 58.8729 30.5267 58.8729 29.9755V5.49418C58.8729 4.94301 58.4259 4.49609 57.8748 4.49609H10.1252C9.57405 4.49609 9.12714 4.94287 9.12714 5.49418V40.1593H6.64673V21.0824C6.64673 20.5312 6.19982 20.0843 5.64865 20.0843C5.09748 20.0843 4.65056 20.5312 4.65056 21.0824V40.1593H0.998086C0.446914 40.1593 0 40.6062 0 41.1574V44.1928C0 46.2824 1.69987 47.9822 3.78941 47.9822H64.2105C66.3 47.9822 67.9999 46.2822 67.9999 44.1928V41.1574C68 40.6061 67.5531 40.1593 67.0019 40.1593ZM66.004 44.1928C66.004 45.1817 65.1995 45.9862 64.2106 45.9862H3.78941C2.80048 45.9862 1.99604 45.1817 1.99604 44.1928V42.1555H24.2122C24.3283 43.34 25.33 44.2688 26.5447 44.2688H38.7822C39.9967 44.2688 40.9986 43.34 41.1146 42.1555H66.004V44.1928Z" fill="white" />
                                <path d="M27.7227 32.2407C27.4479 32.7173 27.6115 33.3265 28.0881 33.6013C29.8781 34.6334 31.9222 35.1787 33.9994 35.1787C40.5341 35.1787 45.8504 29.8623 45.8504 23.3277C45.8504 16.793 40.5341 11.4766 33.9994 11.4766C27.4648 11.4766 22.1484 16.7929 22.1484 23.3277C22.1484 26.0325 23.0837 28.678 24.782 30.777C25.128 31.2047 25.7552 31.2708 26.1829 30.9248C26.6105 30.5788 26.6768 29.9516 26.3307 29.5239C24.8979 27.7531 24.1406 25.6105 24.1406 23.3277C24.1406 17.8915 28.5633 13.4688 33.9994 13.4688C39.4356 13.4688 43.8582 17.8914 43.8582 23.3277C43.8582 28.7638 39.4356 33.1865 33.9994 33.1865C32.271 33.1865 30.5709 32.7331 29.0831 31.8752C28.6067 31.6007 27.9976 31.7641 27.7227 32.2407Z" fill="white" />
                                <path d="M31.7342 29.1174C31.9429 29.1174 32.1503 29.0519 32.3247 28.9235L38.8393 24.1294C39.0943 23.9418 39.2451 23.6439 39.2451 23.3273C39.2451 23.0105 39.0944 22.7126 38.8393 22.5249L32.3247 17.7309C32.0222 17.5083 31.6202 17.4747 31.2848 17.6443C30.9496 17.8138 30.7383 18.1575 30.7383 18.5332V28.1211C30.7383 28.4969 30.9496 28.8406 31.2848 29.01C31.4269 29.0819 31.5808 29.1174 31.7342 29.1174ZM32.7305 20.5031L36.5684 23.3274L32.7305 26.1516V20.5031Z" fill="white" />
                            </svg>
                ...
                <div class="pmt"></div>
                <div class="mt-5 pcategory p-4 shadow" id="fx">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="budget" class="form-control form-select bg-transparent bd @error('budget') is-invalid @enderror" aria-label="Default select example">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="job_duration" id="job_duration" class="form-control form-select bg-transparent bd @error('job_duration') is-invalid @enderror" aria-label="Default select example">
                            @if (old('job_duration'))
                            <option value="{{old('job_duration')}}" selected>{{old('job_duration')}}</option>
                            @endif
        ...
        <div class="form-group mt-4">
            <button id="jobForm-submit" class="btn btn-secondary ml-auto">Yes! Post my job</button>
        </div>
        <!-- ../Project Estimated budget-->
        </form>
        @else
        <div class="p-5">
            <h6>You do not have a Product in your Station</h6>
            <a href="javascript:void(0);" data-toggle="modal" data-target="#uploadProduct" class="btn btn-primary" target="_blank" role="button">
                Add A Product
            </a>
        </div>
...
<script>
    jQuery(document).ready(function() {
        ...
        $('#jobForm-submit').on('click', function() {
            if ($('#jobForm').valid()) {
                $('#jobForm').submit();
            }
        });
