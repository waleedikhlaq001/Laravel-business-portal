...
/**
 * Vicommart Documentation Javascript File
 */
"use strict";
...
( function () {
    ...
    var Docs = {
        init: function () {
            ...
            // # Register events
            Vicommart.$body.on( 'click', '.document-link', function ( e ) {
                self.open( e.currentTarget.getAttribute( 'href' ).slice( 1 ) );
                var mainContent = Vicommart.byClass( 'main-content' );
        ...
        open: function ( id ) {
            ...
            if ( activeId && templates[activeId] ) {
                Vicommart.byId( 'document-view' ).innerHTML = templates[activeId].text;
                Vicommart.byId( 'document-title' ).textContent = templates[activeId].title;
                Vicommart.byId( 'document-view' ).classList.remove( 'search-result' );
            }
            $( Vicommart.byClass( 'document-link' ) ).parent( 'li' ).removeClass( 'show' );
        ...
        search: function ( query ) {
            ...
            Vicommart.byId( 'document-title' ).textContent = 'Search Result';
            ...
            Vicommart.byId( 'document-view' ).innerHTML = finalResult ? finalResult : '<h5 class="text-center">Nothing Found</h5>';
            Vicommart.byId( 'document-view' ).classList.add( 'search-result' );
        }
    }

    Vicommart.docs = Docs;
    $( window ).on( 'wolmart_complete', function () {
        Vicommart.docs && Vicommart.docs.init();
    } )
} )();